      ...
      try {
        ...
        if (secretsDegraded) {
          ...
          logSecrets.info(`[SECRETS_RELOADER_RECOVERED] ${recoveredMessage}`, {
            consoleMessage: `[SECRETS_RELOADER_RECOVERED] ${recoveredMessage} | 密钥解析已恢复，运行时继续沿用故障期间的最近可用快照。`,
          });
        ...
        if (!secretsDegraded) {
          logSecrets.error(`[SECRETS_RELOADER_DEGRADED] ${details}`, {
            consoleMessage: `[SECRETS_RELOADER_DEGRADED] ${details} | 密钥解析失败：如果这是启动阶段，网关会拒绝继续启动；如果是运行中刷新，则继续使用最近一次可用快照。`,
          });
          ...
          logSecrets.warn(`[SECRETS_RELOADER_DEGRADED] ${details}`, {
            consoleMessage: `[SECRETS_RELOADER_DEGRADED] ${details} | 密钥解析仍未恢复，当前继续保持降级运行状态。`,
          });
        }
