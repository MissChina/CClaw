...
export function getSlashCommandDescription(command: SlashCommandDef): string {
  const key = `chat.slash.commands.${command.name}.description`;
  const translated = t(key);
  return translated === key ? command.description : translated;
}

export function getSlashCommandCategoryLabel(category: SlashCommandCategory): string {
  const key = `chat.slash.categories.${category}`;
  const translated = t(key);
  return translated === key ? category.charAt(0).toUpperCase() + category.slice(1) : translated;
}

export function getSlashCommandArgHint(command: SlashCommandDef): string | undefined {
  const key = `chat.slash.commands.${command.name}.args`;
  const translated = t(key);
  return translated === key ? command.args : translated;
}
