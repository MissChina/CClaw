...
export function renderStreamingGroup(
  ...
  const name = assistant?.name ?? t("chat.pinnedAssistant");
...
export function renderMessageGroup(
  ...
  const assistantName = opts.assistantName ?? t("chat.pinnedAssistant");
  const userLabel = group.senderLabel?.trim();
  const who =
    normalizedRole === "user"
      ? (userLabel ?? t("chat.pinnedYou"))
    ...
    <span class="chat-delete-wrap">
      <button
        ...
        @click=${(e: Event) => {
          ...
          const wrap = btn.closest(".chat-delete-wrap");
          if (!wrap) {
            onDelete();
            return;
          }
          ...
          popover.innerHTML = `
            <p class="chat-delete-confirm__text">${t("chat.deleteConfirm")}</p>
            <label class="chat-delete-confirm__remember">
              <input type="checkbox" class="chat-delete-confirm__check" />
              <span>${t("chat.deleteRemember")}</span>
            </label>
            <div class="chat-delete-confirm__actions">
              <button class="chat-delete-confirm__cancel" type="button">${t("chat.cancel")}</button>
              <button class="chat-delete-confirm__yes" type="button">${t("chat.delete")}</button>
            </div>
          `;
...
function renderTtsButton(group: MessageGroup) {
  return html`
    <button
      class="chat-tts-btn"
      type="button"
      title=${isTtsSpeaking() ? t("chat.stopSpeaking") : t("chat.readAloud")}
      aria-label=${isTtsSpeaking() ? t("chat.stopSpeaking") : t("chat.readAloud")}
      @click=${(e: Event) => {
        const btn = e.currentTarget as HTMLButtonElement;
        if (isTtsSpeaking()) {
          stopTts();
          btn.classList.remove("chat-tts-btn--active");
          btn.title = t("chat.readAloud");
          btn.setAttribute("aria-label", t("chat.readAloud"));
          return;
        }
        ...
        btn.title = t("chat.stopSpeaking");
        btn.setAttribute("aria-label", t("chat.stopSpeaking"));
        speakText(text, {
          onEnd: () => {
            if (btn.isConnected) {
              btn.classList.remove("chat-tts-btn--active");
...
function renderAvatar(
  ...
  const assistantName = assistant?.name?.trim() || t("chat.pinnedAssistant");
...
function renderCollapsedToolCards(
  ...
  const summaryLabel =
    toolNames.length <= 3
      ? toolNames.join(", ")
      : t("chat.toolsMore", {
          names: toolNames.slice(0, 2).join(", "),
    ...
    <details class="chat-tools-collapse">
      <summary class="chat-tools-summary">
        ...
        <span class="chat-tools-summary__count">${
          totalTools === 1
            ? t("chat.toolsCount", { count: String(totalTools) })
            : t("chat.toolsCountPlural", { count: String(totalTools) })
        }</span>
...
function jsonSummaryLabel(parsed: unknown): string {
  if (Array.isArray(parsed)) {
    return parsed.length === 1
      ? t("chat.jsonArray", { count: String(parsed.length) })
      : t("chat.jsonArrayPlural", { count: String(parsed.length) });
  }
  if (parsed && typeof parsed === "object") {
    ...
    return t("chat.jsonObject", { count: String(keys.length) });
  }
  return t("chat.jsonLiteral");
}
