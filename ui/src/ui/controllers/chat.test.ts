...
describe("handleChatEvent", () => {
  ...
  it("persists streamed text and an inline error message when a run errors", () => {
    ...
    expect(state.chatMessages).toEqual([
      ...
      {
        role: "assistant",
        content: [{ type: "text", text: "Error: backend exploded" }],
        timestamp: 2_000,
      },
    ]);
    expect(state.chatRunId).toBeNull();
    expect(state.chatStream).toBeNull();
    expect(state.lastError).toBe("backend exploded");
    nowSpy.mockRestore();
  });
});
