...
function renderFallbackIndicator(status: FallbackIndicatorStatus | null | undefined) {
  ...
  const details =
    phase === "cleared"
      ? t("chat.fallbackDetailsCleared", {
          selected: status.selected,
          active: status.selected,
          previous: status.previous ?? "-",
        })
      : t("chat.fallbackDetailsActive", {
          selected: status.selected,
          active: status.active,
        });
  const message =
    phase === "cleared"
      ? t("chat.fallbackCleared", { active: status.selected })
      ...
      ${attachments.map(
        (att) => html`
          <div class="chat-attachment-thumb">
            <img src=${att.dataUrl} alt=${t("chat.attachPreview")} />
            <button
...
function getWelcomeSuggestions() {
  return [
    t("chat.welcomeSuggestionWhat"),
    t("chat.welcomeSuggestionSessions"),
    t("chat.welcomeSuggestionChannel"),
    t("chat.welcomeSuggestionHealth"),
  ];
}
...
function renderWelcomeState(props: ChatProps): TemplateResult {
  const name = props.assistantName || t("chat.pinnedAssistant");
  ...
  return html`
    <div class="agent-chat__welcome" style="--agent-color: var(--accent)">
      ...
      <div class="agent-chat__badges">
        <span class="agent-chat__badge"><img src=${logoUrl} alt="" /> ${t("chat.welcomeReady")}</span>
        <span class="agent-chat__badge agent-chat__badge--muted">${t("chat.welcomeSlashBadge")}</span>
      </div>
      <p class="agent-chat__hint">
        ${unsafeHTML(t("chat.welcomeHint"))}
      </p>
      <p class="agent-chat__hint agent-chat__hint--secondary">${t("chat.welcomeSubhint")}</p>
...
function renderSearchBar(requestUpdate: () => void): TemplateResult | typeof nothing {
  ...
  return html`
    <div class="agent-chat__search-bar">
      ...
      />
      <button class="btn-ghost" title=${t("chat.searchClose")} aria-label=${t("chat.searchClose")} @click=${() => {
        vs.searchOpen = false;
        vs.searchQuery = "";
        requestUpdate();
      }}>
            ...
            <div class="agent-chat__pinned-list">
              ${entries.map(
                ...
                <div class="agent-chat__pinned-item">
                  <span class="agent-chat__pinned-role">${role === "user" ? t("chat.pinnedYou") : t("chat.pinnedAssistant")}</span>
                  <span class="agent-chat__pinned-text">${text.slice(0, 100)}${text.length > 100 ? "..." : ""}</span>
                  <button class="btn-ghost" @click=${() => {
                    pinned.unpin(index);
                    requestUpdate();
                  }} title=${t("chat.pinnedUnpin")} aria-label=${t("chat.pinnedUnpin")}>
                    ${icons.x}
                  </button>
                </div>
              `,
      ...
      <div class="slash-menu">
        <div class="slash-menu-group">
          <div class="slash-menu-group__label">
            ${t("chat.slash.argMenuTitle", {
              command: `/${vs.slashMenuCommand.name}`,
              description: getSlashCommandDescription(vs.slashMenuCommand),
            })}
          </div>
          ${vs.slashMenuArgItems.map(
            (arg, i) => html`
              ...
              >
                ...
                <span class="slash-menu-desc">${t("chat.slash.argMenuValue", {
                  command: `/${vs.slashMenuCommand?.name}`,
                  value: arg,
                })}</span>
              </div>
        ...
        <div class="slash-menu-footer">
          ...
          <kbd>Tab</kbd> ${t("chat.slashFill")}
          <kbd>Enter</kbd> ${t("chat.slashRun")}
          <kbd>Esc</kbd> ${t("chat.slashClose")}
        </div>
      </div>
    `;
  }
        ...
        ${entries.map(
          ({ cmd, globalIdx }) => html`
            ...
            >
              ...
              ${getSlashCommandArgHint(cmd) ? html`<span class="slash-menu-args">${getSlashCommandArgHint(cmd)}</span>` : nothing}
              <span class="slash-menu-desc">${getSlashCommandDescription(cmd)}</span>
              ${
                cmd.argOptions?.length
                  ? html`<span class="slash-menu-badge">${t("chat.slashOptions", { count: String(cmd.argOptions.length) })}</span>`
                  : cmd.executeLocal && !cmd.args
                    ? html`
                        <span class="slash-menu-badge">${t("chat.slashInstant")}</span>
                      `
                    : nothing
              }
            </div>
  ...
  return html`
    <div class="slash-menu">
      ...
      <div class="slash-menu-footer">
        <kbd>↑↓</kbd> ${t("chat.slashNavigate")}
        <kbd>Tab</kbd> ${t("chat.slashFill")}
        <kbd>Enter</kbd> ${t("chat.slashSelect")}
        <kbd>Esc</kbd> ${t("chat.slashClose")}
      </div>
    </div>
  `;
...
export function renderChat(props: ChatProps) {
  ...
  const placeholder = props.connected
    ? hasAttachments
      ...
      : t("chat.inputPlaceholderMessage", { name: props.assistantName || "agent" })
    ...
    >
      ...
      ${
        isEmpty && vs.searchOpen
          ? html`
              <div class="agent-chat__empty" role="status" aria-live="polite">
                <div class="agent-chat__empty-title">${t("chat.searchEmptyTitle")}</div>
                <div class="agent-chat__empty-copy">${t("chat.searchEmptyBody", {
                  query: vs.searchQuery || t("chat.searchPlaceholder"),
                })}</div>
              </div>
            `
          : nothing
    ...
    >
        ...
        props.queue.length
          ? html`
            <div class="chat-queue" role="status" aria-live="polite">
              <div class="chat-queue__title">${t("chat.queueTitle", { count: String(props.queue.length) })}</div>
                      ...
                      <div class="chat-queue__text">
                        ${
                          ...
                          (item.attachments?.length
                            ? t("chat.queueImage", { count: String(item.attachments.length) })
                            : "")
                        }
                      </div>
                      <button
      ...
      <div class="agent-chat__input">
        ...
        <textarea
          ...
          placeholder=${vs.sttRecording ? t("chat.inputListening") : placeholder}
          rows="1"
        ></textarea>
        ...
        <div class="agent-chat__toolbar">
              ...
              isSttSupported()
                ? html`
                  <button
                    ...
                    aria-label=${vs.sttRecording ? t("chat.stopRecording") : t("chat.voiceInput")}
                    ?disabled=${!props.connected}
                  >
          ...
          <div class="agent-chat__toolbar-right">
            ...
            <button class="btn-ghost" @click=${() => exportMarkdown(props)} title=${t("chat.export")} aria-label=${t("chat.export")} ?disabled=${props.messages.length === 0}>
              ${icons.download}
            </button>
            ...
            ${
              canAbort && (isBusy || props.sending)
                ? html`
                  <button class="chat-send-btn chat-send-btn--stop" @click=${props.onAbort} title=${t("chat.stop")} aria-label=${t("chat.stop")}>
                    ${icons.stop}
                  </button>
                `
                : html`
  ...
  for (let i = historyStart; i < history.length; i++) {
    ...
    if (marker && marker.kind === "compaction") {
      items.push({
        ...
        label: t("chat.compactionDivider"),
