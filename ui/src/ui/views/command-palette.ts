...
function getPaletteItemsInternal(): PaletteItem[] {
  const slashPaletteItems: PaletteItem[] = SLASH_COMMANDS.map((command) => ({
  ...
  return [
    ...
    {
      id: "skill-shell",
      label: t("overview.palette.shellLabel"),
      icon: "monitor",
      category: "skills",
      action: "/skill shell",
      description: t("overview.palette.shellDescription"),
    },
    {
      id: "skill-debug",
      label: t("overview.palette.debugLabel"),
      icon: "bug",
      category: "skills",
      action: "/verbose full",
      description: t("overview.palette.debugDescription"),
    },
  ];
}
...
function getPaletteCategoryLabel(category: string): string {
  const key = `overview.palette.categories.${category}`;
          ...
          ${
            grouped.length === 0
              ...
              : grouped.map(
                ...
                <div class="cmd-palette__group-label">${getPaletteCategoryLabel(category)}</div>
