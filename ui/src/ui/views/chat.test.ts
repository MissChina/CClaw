...
describe("chat view", () => {
  ...
  it("switches welcome copy to zh-CN", async () => {
    await i18n.setLocale("zh-CN");
    const container = document.createElement("div");
    render(renderChat(createProps()), container);

    expect(container.textContent).toContain("可以开始聊天了");
    expect(container.textContent).toContain("你能做什么？");
  });
  ...
  it("opens the file picker from the active chat composer only", () => {
    const hostA = document.createElement("div");
    const hostB = document.createElement("div");
    document.body.append(hostA, hostB);
    render(renderChat(createProps()), hostA);
    render(renderChat(createProps()), hostB);

    const inputA = hostA.querySelector<HTMLInputElement>(".agent-chat__file-input");
    const inputB = hostB.querySelector<HTMLInputElement>(".agent-chat__file-input");
    const clickA = vi.spyOn(inputA!, "click");
    const clickB = vi.spyOn(inputB!, "click");

    hostB.querySelector<HTMLButtonElement>(".agent-chat__input-btn")?.click();
  ...
  it("renders fallback-cleared indicator shortly after transition", () => {
    ...
    expect(indicator).not.toBeNull();
    expect(indicator?.textContent).toContain("Fallback restored: fireworks/minimax-m2p5");
    nowSpy.mockRestore();
  });
